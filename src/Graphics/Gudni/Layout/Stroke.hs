...
segmentedRectangle height ls =
  let xs = scanlChain (+) 0 ls
      ...
      mkLine y (start, end) = straight (Point2 start y) (Point2 end y)
      top = fmap (mkLine 0) segments
      bottom = reverseChain . fmap (mkLine height) $ reverseSegments
      startCap = straight (lastLink bottom ^. bzEnd) (firstLink top ^. bzStart)
      endCap = straight (lastLink top ^. bzEnd) (firstLink bottom ^. bzStart)
...
instance Space s => CanStroke (BezierSpace s) where
  ...
  strokeOffset offset thickness bSpace =
    ...
    in  projection bSpace . translateByXY 0 (Ortho offset) $ rect
