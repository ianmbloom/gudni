...
module Graphics.Gudni.Raster.ConfineTree.ConfineTree
  ( ConfineTree (..)
  , Confine(..)
  , confineItemTagId
  , confineCurveTag
...
crossCurve anchor point stack (itemTagId, bez) =
    if --tc ("crossCurve anchor " ++ show anchor ++ " point "++ show point) $
...
traverseCurve anchor point box tree stack =
  ...
  then crossCurve anchor point stack (itemTagId, bez)
  else stack
