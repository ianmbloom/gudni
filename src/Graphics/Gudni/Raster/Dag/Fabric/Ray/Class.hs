...
class ( HasSpace r
    ...
    overTree      :: MonadIO m => ConfineTree (SpaceOf r) -> DecorateTree (SpaceOf r) -> r -> RayMonad (SpaceOf r) m ShapeStack
    overTransform :: FTransformer (SpaceOf r) -> r -> r
    overFacet     :: Facet (SpaceOf r) -> r -> r

instance (Space s, Storable s) => Ray (Point2 s) where
    rayToPoint                ray = ray
    overTree      cTree dTree ray = lift $ queryConfineTreePoint loadPrimS cTree dTree (rayToPoint ray)
