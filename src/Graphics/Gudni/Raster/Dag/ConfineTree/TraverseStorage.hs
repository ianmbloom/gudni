...
module Graphics.Gudni.Raster.Dag.ConfineTree.TraverseStorage
  ( traverseCTagAlong
  , traverseCTagBetweenPoints
  , traverseCTagBox
  , traverseDecorateTreeTag
  )
where
...
traverseCTagAlong :: forall lineAxis s m f
                  .  ( Axis lineAxis
                     , Space s
                     , Storable s
                     , MonadIO m
                     , MonadTrans f
                     , Monad (f (DagMonad s m))
                     )
                  => (PrimTagId -> f (DagMonad s m) ())
                  -> lineAxis
                  -> Along   lineAxis s
                  -> Athwart lineAxis s
                  -> Along   lineAxis s
                  -> ConfineTagId s
                  -> f (DagMonad s m) ()
traverseCTagAlong f lineAxis start baseline end =
...
traverseCTagBox f box tree =
    ...
    go axis treeId =
      ...
      then return ()
      else do tree <- lift $ loadTreeTagS treeId
...
traverseDecorateTreeTag :: forall s m f
                        .  ( Space s
                           , Storable s
                           , MonadIO m
                           , MonadTrans f
                           , MonadIO (f (DagMonad s m))
                           )
                        => (Slice ShapeId -> f (DagMonad s m) ())
                        -> (Point2 s      -> f (DagMonad s m) ())
                        -> Point2 s
                        -> DecoTagId s
                        -> f (DagMonad s m) ()
traverseDecorateTreeTag doBranch doLeaf point =
  ...
  go axis parentCut parentLine treeId =
      ...
      then let anchor = pointAlongAxis (perpendicularTo axis) parentCut parentLine
           in  doLeaf anchor
      else do tree <- lift $ loadDecoTagS treeId
