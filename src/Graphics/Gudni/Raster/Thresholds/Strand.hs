...
module Graphics.Gudni.Raster.Thresholds.Strand
  ( StrandElement_(..)
  , Strand(..)
  , outlineToStrands
  )
...
splitShape table maxSectionSize beziers =
    ...
    join .
    fmap (replaceKnob Vertical) $   -- Split curve sections that bulge in the x direction to two curve sections that do not bulge.
    beziers
