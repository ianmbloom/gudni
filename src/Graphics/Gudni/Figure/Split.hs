...
findSplit :: forall s . (Show s, Fractional s, Ord s, Num s, Iota s) => (Point2 s -> s) -> s -> Bezier s -> (Bezier s, Bezier s)
findSplit axis cut bezier@(Bez v0 control v1) = search 0.0 1.0 0.5
  ...
  search :: s -> s -> s -> (Bezier s, Bezier s)
  search lower upper t
    ...
    where innerBezier@(Bez mid0 onCurve mid1) = insideBezier t bezier
          ...
          final = (Bez v0 mid0 onCurve, Bez onCurve mid1 v1)


-- | Split a curve across a vertical axis
splitBezierX :: (Space s) => s -> Bezier s -> (Bezier s, Bezier s)
